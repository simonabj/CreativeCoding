...
class GladosTTS:
    def __init__(self, use_p1: bool=False, log: bool=False):
        ...
        if use_p1:
            self.emb = torch.load(f'{self.model_dir}/emb/glados_p1.pt')
        else:
            self.emb = torch.load(f'{self.model_dir}/emb/glados_p2.pt')
        ...
        # Load models
        self.glados = torch.jit.load(f'{self.model_dir}/glados-new.pt')
        self.vocoder = torch.jit.load(f'{self.model_dir}/vocoder-gpu.pt', map_location=self.device)
    ...
    def run_tts(self, text, output_file: str="output.wav", alpha: float=1.0) -> AudioSegment:
        x = prepare_text(text)
    ...
    def speak(self, text, alpha: float=1.0, delay: float=0.1, play: bool=False):
        ...
        self.speak_one_line(old_line, "old_line.wav", play=play)
        old_time = time.time()
        ...
        if len(sentences) > 1:
            for idx in range(1, len(sentences)):
                ...
                if idx % 2 == 1:
                    self.speak_one_line(new_line, "new_line.wav", play=play)
                else:
                    self.speak_one_line(old_line, "old_line.wav", play=play)
                old_time = time.time()
                old_dur = new_dur
        ...
        time_left = old_dur - time.time() + old_time
        if play and time_left >= 0:
            time.sleep(time_left + delay)

if __name__ == "__main__":
    glados = GladosTTS(False, True)
    while True:
        text = input("Input: ")
        if len(text) > 0:
            glados.run(text, True, play=true)